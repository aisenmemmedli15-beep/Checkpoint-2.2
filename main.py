...
def ingest_and_chunk_documents(file_path: str):
    ...
    print(f" Ümumi yaradılmış chunk sayı: {len(chunks)}")
    print("-" * 50)
...
def create_vector_store(chunks):
    """CHECKPOINT 2: Chunk-lar üçün embedding generasiyası və FAISS vektor bazası."""
    print(" Chunk-lar üçün embedding-lər generasiya olunur...")
    embeddings = OpenAIEmbeddings()

    print(" FAISS vektor bazası yaradılır və indekslənir...")
...
def main():
    ...
    # 2. EXECUTE CHECKPOINT 2: Embedding Generation & FAISS Vector Store
    vectorstore = create_vector_store(chunks)

    # 3. Testlər
    print("Test Sorğuları İcra Olunur:\n")
